...
public class InventoryItem extends POSModel implements java.io.Serializable {
	...
	public Object[] toObjectArray(boolean includeId) {
		...
		if (includeId) {
			Object[] params = { this.productId, this.trackingCode,
					this.quantity, this.receivedDate, this.expiryDate,
					this.promotionalOffer, this.id };
			return params;
		} else {
			Object[] params = { this.productId, this.trackingCode,
					this.quantity, this.receivedDate, this.expiryDate,
	...
	public final boolean verifyRequiredFields() {
		...
		if (this.trackingCode != null && this.trackingCode.trim().length() > 0
				&& this.promotionalOffer != null && this.promotionalOffer.trim().length() > 0
				&& this.receivedDate != null && this.expiryDate != null
				&& this.quantity != 0
				&& this.productId != 0) {
			return true;
		}
